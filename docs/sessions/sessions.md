...
本文件是本仓库：**`session_id` 语义**、**执行中哪些环节使用**、**在 Markdown 中如何标注**、**推荐字面值与归档路径及工具**的**唯一权威说明**；其它文档（含 [INDEX.md](../../INDEX.md)、[documentation-standards.md](../documentation-standards.md)）**只引用本文**，口径冲突以**本文**为准。与拼图边界见 [core-logic.md](../core-logic.md)。**非 `session_id` 的文档/路径基名规则**见 [documentation-standards.md](../documentation-standards.md) **文档 · 文档与路径命名规范** 的「其它文档」。
...
| **name** | CLI **`--name`**（`--title` 等价）人工概括，最多 **5 个词**；路径安全化与「其它文档」基名的 `name` 规则一致，见 [documentation-standards.md](../documentation-standards.md) **文档 · 文档与路径命名规范**。 |
