  ...
  describe('validate() — invalid documents', () => {
    ...
    it('rejects blank workflow-run lineage identifiers across node execution and replay state', () => {
      const cases = [
        ...
        ['capsule_state.node_id', (doc, value) => { doc.data.node_execution.capsule_state.node_id = value; }],
        ['capsule_state.lease.holder', (doc, value) => { doc.data.node_execution.capsule_state.lease.holder = value; }],
    ...
    it('rejects blank workflow-run report actor values', () => {
      for (const value of ['', '   ']) {
        const doc = buildWorkflowRunDoc();
        doc.data.reports[0].actor = value;
        const { valid } = validator.validate('workflow-run', doc);
        assert.equal(valid, false, `Expected invalid for reports[0].actor=${JSON.stringify(value)}`);
      }
    });

    it('rejects blank workflow-run report step ids', () => {
      for (const value of ['', '   ']) {
        const doc = buildWorkflowRunDoc();
        doc.data.reports[0].step_id = value;
        const { valid } = validator.validate('workflow-run', doc);
        assert.equal(valid, false, `Expected invalid for reports[0].step_id=${JSON.stringify(value)}`);
      }
    });
    ...
    it('rejects blank workflow-run step ids', () => {
      for (const value of ['', '   ']) {
        const doc = buildWorkflowRunDoc();
        doc.data.steps[0].step_id = value;
        const { valid } = validator.validate('workflow-run', doc);
        assert.equal(valid, false, `Expected invalid for steps[0].step_id=${JSON.stringify(value)}`);
      }
    });
    ...
    it('rejects blank workflow-run report commit sha values', () => {
      for (const value of ['', '   ']) {
        const doc = buildWorkflowRunDoc();
        doc.data.reports[0].commit_sha = value;
        const { valid } = validator.validate('workflow-run', doc);
        assert.equal(valid, false, `Expected invalid for reports[0].commit_sha=${JSON.stringify(value)}`);
      }
    });
